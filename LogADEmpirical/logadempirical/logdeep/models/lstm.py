...
class deeplog(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, vocab_size, embedding_dim, window_size):
...
class robustlog(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, vocab_size, embedding_dim, window_size, num_keys=2):
...
class loganomaly(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, vocab_size, embedding_dim, window_size):
        ...
        self.lstm0 = nn.LSTM(window_size,
                            hidden_size,
                            num_layers,
                            batch_first=True)

        self.lstm1 = nn.LSTM(vocab_size,
                             hidden_size,
                             num_layers,
                             batch_first=True)
    ...
    def forward(self, features, device):
        # print(len(features)")
        input0, input1 = features[0].to(torch.float32), features[1].to(torch.float32)
        # print(input1.shape
        ...
        h0_1 = torch.zeros(self.num_layers, input1.size(0),
                           self.hidden_size, dtype=input1.dtype).to(device)
