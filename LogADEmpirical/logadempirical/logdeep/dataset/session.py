...
def session_window_bgl(raw_data):
    ...
    for idx, row in tqdm(enumerate(raw_data)):
        ...
        if node_id not in data_dict.keys():
            ...
            data_dict[node_id]['Seq'] = [row['Content']]
    ...
    for k, v in data_dict.items():
        results.append({"SessionId": k, "EventId": v['EventId'], "Seq": v['Seq'], "Label": np.array(label_dict[k])})
    results = shuffle(results)
    return results
...
def sliding_window(raw_data, para):
    ...
    # print(label_data[:10])
    logkey_data, deltaT_data, log_template_data, content_data = raw_data.iloc[:, 2], raw_data.iloc[:, 3], raw_data.iloc[:, 4], raw_data.iloc[:, 5]
    new_data = []
    ...
    for (start_index, end_index) in start_end_index_pair:
        ...
        new_data.append([
            ...
            content_data[start_index: end_index]
        ])
    assert len(start_end_index_pair) == len(new_data)
    print('There are %d instances (sliding windows) in this dataset' % len(start_end_index_pair))
    return pd.DataFrame(new_data, columns=['timestamp', 'Label', 'EventId', 'deltaT', 'EventTemplate', 'Seq'])
