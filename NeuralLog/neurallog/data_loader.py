...
embed_dim = 256
dense_layer = keras.layers.Dense(embed_dim)
...
def load_supercomputers(log_file, train_ratio=0.5, windows_size=20, step_size=0, e_type='bert', mode="balance",
                        no_word_piece=0, sampling_method ='N', sampling_ratio=1):
    ...
    while i < n_train - windows_size:
        ...
        for j in range(i, i + windows_size):
            ...
            emb = E[content]
            # emb=tf.cast(emb, tf.float16)
            seq.append(emb)

        x_tr.append(tf.concat(seq, axis = 0))
    ...
    if sampling_method == "SMOTE":
        ...
        for i in range(len(x_tr)):
            if i in addIndex:
                os_train_logs.append(x_tr[i])
                os_train_labels.append(y_tr[i])
            else:
                continue
        x_tr = x_tr + os_train_logs
        y_tr = y_tr + os_train_labels
        ...
        for i in range(len(y_tr)):
            if i in removeIndex:
                ...
                us_train_logs.append(x_tr[i])
                us_train_labels.append(y_tr[i])
        x_tr = us_train_logs
        y_tr = us_train_labels

    if sampling_method != 'N' and sampling_method != 'randomU' and sampling_method != 'randomO':
        print(f'Sampling ratio: {per}')
        x_tr, y_tr = sm.fit_resample(x_tr, y_tr)
    
    x_tr = [tf.reshape(x, [windows_size, -1]) for x in x_tr]
